    ...
    class InternalNode extends Node {
        ...
        public Integer search(int key) {
            ...
            while (currentNode != null) {
                int i = 0;
                while (i < currentNode.keyCount && currentNode.keys[i] < key) {
                    i++;
                }

                // 키가 일치하면 반환
                if (i < currentNode.keyCount && currentNode.keys[i] == key) {
                    return currentNode.keys[i];
                }

                // 자식 노드가 있다면 해당 자식 노드로 이동
                if (i < currentNode.keyCount && currentNode.getChild(i) != null) {
                    currentNode = currentNode.getChild(i); // 자식 노드로 내려감
                } else {
                    return null; // 자식이 없거나 값이 없으면 종료
                }
            }
            ...
            if (children[0] != null) {
                ...
                for (int i = 0; i < keyCount; i++) {
                    if (start <= keys[i]) {
                        ...
                        if (children[i] != null) {
                            result.addAll(children[i].search(start, end)); // 자식 노드로 내려가기
                        }
                    }
                }
                // 마지막 자식 노드도 범위에 맞게 검색
                if (children[keyCount] != null) {
                    result.addAll(children[keyCount].search(start, end));
                }
            } else {
                // 리프 노드에 도달한 경우
                for (int i = 0; i < keyCount; i++) {
                    if (keys[i] >= start && keys[i] <= end) {
                        result.add(keys[i]);  // 범위 내의 값을 결과에 추가
                    }
                }
            }
        ...
        public void split(int index, Node child) {
            ...
            for (int i = 0; i < degree - 1; i++) {
                newNode.setKey(i, child.getKey(i + degree));
            }
            for (int i = 0; i < degree; i++) {
                newNode.setChild(i, child.getChild(i + degree));
            }
